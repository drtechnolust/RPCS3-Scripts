    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($hwndInstall -ne [IntPtr]::Zero -and [User32]::IsWindowVisible($hwndInstall)) {
            ...
            try {
                ...
                # Search the UI tree for the checkbox
                ...
                # If UIAutomation fails for any reason, it will safely ignore and just hit enter anyway
            }
            ...
            $clicked = $true
            Write-Host "`r    Installing — waiting for RPCS3 to finish...                    " -ForegroundColor DarkGreen
            break 
        }
    }

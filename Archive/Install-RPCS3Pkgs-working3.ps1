...
function Write-LogRow {
    param(
        ...
        [string] $Status, [string] $Details, [string] $DetectedGameFolder, [int] $ExitCode
    )
    $row = [pscustomobject] @{
        Timestamp = (Get-Date -Format 'yyyy-MM-dd HH:mm:ss')
        ArchivePath = $ArchivePath; PkgPath = $PkgPath; RapPath = $RapPath
        Status = $Status; Details = $Details
        DetectedGameFolder = $DetectedGameFolder; ExitCode = $ExitCode
    }
...
function Get-GameSnapshot ([string] $Path) {
    ...
    if (Test-Path -LiteralPath $Path) {
        Get-ChildItem -LiteralPath $Path -Directory | ForEach-Object {
            ...
            $snapshot[$_.FullName] = (Test-Path -LiteralPath $sfo) ? (Get-Item -LiteralPath $sfo).LastWriteTimeUtc : $_.LastWriteTimeUtc
        }
    }
    return $snapshot
}
...
function Get-MatchingRap ([System.IO.FileInfo] $Pkg, [System.IO.FileInfo[]] $Raps) {
    ...
    $exact   = $Raps | Where-Object { [System.IO.Path]::GetFileNameWithoutExtension($_.Name) -eq $pkgBase } | Select-Object -First 1
...
function Invoke-RPCS3Install {
    ...
    while ((Get-Date) -lt $deadline) {
        ...
        if ($hwndLoading -ne [IntPtr]::Zero -and [User32]::IsWindowVisible($hwndLoading)) {
            Write-Host "`r    [+] Auto-canceling 'Loading games' to save time...             " -NoNewline -ForegroundColor DarkYellow
            try {
                $loadElem = [System.Windows.Automation.AutomationElement]::FromHandle($hwndLoading)
                $cancelCond = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::NameProperty, "Cancel")
                $cancelBtn = $loadElem.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $cancelCond)
                if ($cancelBtn) {
                    $invPattern = $cancelBtn.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern) -as [System.Windows.Automation.InvokePattern]
                    if ($invPattern) { $invPattern.Invoke(); Start-Sleep -Milliseconds 500 }
                }
            } catch {}
            $deadline = (Get-Date).AddSeconds($DialogTimeoutSeconds)
            continue 
        ...
        if ($hwndInstall -ne [IntPtr]::Zero -and [User32]::IsWindowVisible($hwndInstall)) {
            ...
            try {
                ...
                $cond = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::NameProperty, "Precompile caches")
            ...
            # --- POST-INSTALL WATCHER (WITH AUTO-CANCEL) ---
            ...
            while ((Get-Date) -lt $installTimeout) {
                ...
                if ($hwndLoad -ne [IntPtr]::Zero -and [User32]::IsWindowVisible($hwndLoad)) {
                    Write-Host "`r    [+] Auto-canceling post-install cache update...                " -NoNewline -ForegroundColor DarkYellow
                    try {
                        $loadElem = [System.Windows.Automation.AutomationElement]::FromHandle($hwndLoad)
                        $cancelCond = [System.Windows.Automation.PropertyCondition]::new([System.Windows.Automation.AutomationElement]::NameProperty, "Cancel")
                        $cancelBtn = $loadElem.FindFirst([System.Windows.Automation.TreeScope]::Descendants, $cancelCond)
                        if ($cancelBtn) {
                            $invPattern = $cancelBtn.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern) -as [System.Windows.Automation.InvokePattern]
...
if ($scanZips) {
    ...
    foreach ($z in $zips) {
        ...
        $workItems.Add([pscustomobject] @{
            Type = 'zip'; SourcePath = $z.FullName; DisplayName = $z.BaseName; SizeBytes = $z.Length
        })
    }
}
...
if ($scanFolders -and ($batchLimit -eq 0 -or $workItems.Count -lt $batchLimit)) {
    ...
    foreach ($d in $dirs) {
        ...
        if ($hasPkg) {
            $workItems.Add([pscustomobject] @{
                Type = 'folder'; SourcePath = $d.FullName; DisplayName = $d.Name; SizeBytes = 0
            })
        }
    }
}
...
if ($workItems.Count -eq 0) {
    Write-Host ('  No installable items found in {0}.' -f ($RetryFailed ? '_Failed' : 'source folder')) -ForegroundColor Yellow
    return
}

# FIXED: Wrapped in @() to prevent Strict-Mode crash if no items are found
$zipCount    = @($workItems | Where-Object Type -eq 'zip').Count
$folderCount = @($workItems | Where-Object Type -eq 'folder').Count
$total       = $workItems.Count

Write-Host ("  Found {0} item(s) to process  ({1} ZIP, {2} folder)." -f $total, $zipCount, $folderCount) -ForegroundColor White
Write-Host ''
...
foreach ($item in $workItems) {
    ...
    if ($isZip) {
        ...
        $firstPkg = Get-ChildItem -LiteralPath $sourcePath -Recurse -File -Filter *.pkg | Select-Object -First 1
        if ($firstPkg) {
            $keyString = '{0}|{1}|{2}' -f $dispName, $firstPkg.Name, $firstPkg.Length
            $bytes     = [System.Text.Encoding]::UTF8.GetBytes($keyString)
            $itemHash  = [System.Convert]::ToHexString([System.Security.Cryptography.SHA256]::HashData($bytes))
        }
    }
    ...
    if (-not $ForceReinstall -and -not $RetryFailed -and $itemHash -and $state.ContainsKey($itemHash)) {
        ...
        $counters.Skipped++
        Write-LogRow -ArchivePath $sourcePath -PkgPath '' -RapPath '' -Status 'Skipped' -Details 'Already in state file' -DetectedGameFolder $cachedFolder -ExitCode 0
        continue
    }
    ...
    try {
        ...
        # FIXED: Wrapped in @() to prevent Strict-Mode crash
        $pkgFiles = @(Get-ChildItem -LiteralPath $workFolder -Recurse -File -Filter *.pkg -ErrorAction SilentlyContinue)
        $rapFiles = @(Get-ChildItem -LiteralPath $workFolder -Recurse -File -Filter *.rap -ErrorAction SilentlyContinue)
        ...
        if (-not $pkgFiles) {
            ...
            Write-LogRow -ArchivePath $sourcePath -PkgPath '' -RapPath '' -Status 'Failed' -Details 'No PKG found' -DetectedGameFolder '' -ExitCode -1
        ...
        if ($DryRun) {
            foreach ($pkg in $pkgFiles) {
                ...
                Write-LogRow -ArchivePath $sourcePath -PkgPath $pkg.FullName -RapPath ($rap?.FullName ?? '') -Status 'DryRun' -Details 'No changes made' -DetectedGameFolder '' -ExitCode 0
            }
            if ($needsCleanup) { Remove-FolderSafe -Path $workFolder }
            continue
        }
        ...
        foreach ($pkg in $pkgFiles) {
            ...
            if ($rap) {
                ...
                if (-not (Test-Path -LiteralPath $rapDestPath)) { throw "RAP copy failed — file missing at destination: $rapDestPath" }
            ...
            $detectedFolder = Wait-ForNewGameFolder -Before $before -InstallStartUtc $installStartUtc -TimeoutSeconds $FolderPollSeconds
            ...
            if ($exitCode -eq 0 -and $detectedFolder) {
                ...
                Write-LogRow -ArchivePath $sourcePath -PkgPath $pkg.FullName -RapPath $rapDestPath -Status 'Success' -Details 'RPCS3 exit 0, folder detected' -DetectedGameFolder $detectedFolder -ExitCode $exitCode
            } else {
                $itemSuccess = $false
                $reason = ($exitCode -ne 0) ? "RPCS3 exit code $exitCode" : "No game folder detected after ${FolderPollSeconds}s poll"
                ...
                Write-LogRow -ArchivePath $sourcePath -PkgPath $pkg.FullName -RapPath $rapDestPath -Status 'Failed' -Details $reason -DetectedGameFolder ($detectedFolder ?? '') -ExitCode $exitCode
            }
        }
        ...
        if ($itemSuccess) {
            ...
            if ($itemHash) {
                $state[$itemHash] = @{ Timestamp = (Get-Date -Format 'o'); Type = $typeLabel; SourcePath = $sourcePath; ArchiveMovedTo = $movedTo; DetectedGameFolder = ($detectedFolders -join '; ') }
        ...
        Write-LogRow -ArchivePath $sourcePath -PkgPath '' -RapPath '' -Status 'Error' -Details $errMsg -DetectedGameFolder '' -ExitCode -1

        if ($isZip -and (Test-Path -LiteralPath $sourcePath)) { Move-ToFolder -Path $sourcePath -Destination $FailedRoot | Out-Null }
        if ($needsCleanup) { Remove-FolderSafe -Path $workFolder }
    }
}
...
Write-Host ("  ✖  Failed       : {0}" -f $totalFailed) -ForegroundColor ($totalFailed -gt 0 ? 'Red' : 'Green')

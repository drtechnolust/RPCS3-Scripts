...
# Nintendo Switch ROM Filename Cleaner - Enhanced Version
...
function Extract-RegionAndVersion {
    ...
    # Extract region - multiple patterns supported
    $region = $null
    $regionPatterns = @(
        '__([A-Z]{2,3})_',    # __US_ format
        '\[([A-Z]{2,3})\]',   # [US] format  
        '\(([A-Z]{2,3})\)'    # (US) format
    )
    
    foreach ($pattern in $regionPatterns) {
        $match = [regex]::Match($nameWithoutExt, $pattern)
        if ($match.Success) {
    ...
    # Extract version - multiple patterns supported
    $version = $null
    $versionPatterns = @(
        '_v(\d+(?:\.\d+)*(?:\.\d+)?)_?',      # _v1.0.0_ or _v1_
        '\[v(\d+(?:\.\d+)*(?:\.\d+)?)\]',     # [v1.0.0]
        '\(v(\d+(?:\.\d+)*(?:\.\d+)?)\)',     # (v1.0.0)
        '_\+_v(\d+(?:\.\d+)*(?:\.\d+)?)_',    # _+_v1.0.1_ (new pattern)
        '\+_v(\d+(?:\.\d+)*(?:\.\d+)?)_'      # +_v1.0.1_ (variant)
    )
...
function Clean-Filename {
    ...
    # Enhanced patterns to extract game name
    $gameName = $nameWithoutExt
    $patterns = @(
        '^(.+?)_[0-9A-Fa-f]{12,}.*$',      # Hex codes (original pattern)
        '^(.+?)__[A-Z]{2,3}_.*$',          # Region markers  
        '^(.+?)\s*\[.*\].*$',              # Anything in brackets
        '^(.+?)\s*\(.*\).*$',              # Anything in parentheses
        '^(.+?)_\+_v\d+.*$',               # New: _+_v1.0.1_ pattern
        '^(.+?)\+_v\d+.*$',                # New: +_v1.0.1_ pattern (variant)
        '^(.+?)_\(nsw2u\).*$'              # New: _(nsw2u) pattern
    )
    
    foreach ($pattern in $patterns) {
        $match = [regex]::Match($gameName, $pattern)
        if ($match.Success) {
    ...
    # If no pattern matched, try more aggressive splitting
    if ($gameName -eq $nameWithoutExt) {
        # Split on underscores and take parts until we hit metadata
        $parts = $gameName -split '_'
        if ($parts.Count -gt 1) {
            $cleanParts = @()
            foreach ($part in $parts) {
                # Stop at hex codes, known metadata, or version info
                if ($part -match '^[0-9A-Fa-f]{8,}$' -or 
                    $part.ToLower() -in @('nsw2u', 'patched', 'base', 'update') -or
                    $part -match '^\+$' -or
                    $part -match '^v\d+' -or
                    $part -match '^\(' -or
                    $part -match '-\d+$') {  # Handles -1 suffix
                    break
                }
                $cleanParts += $part
            }
            if ($cleanParts.Count -gt 0) {
                $gameName = $cleanParts -join '_'
            }
        }
    }
...
function Process-GameFiles {
    ...
    # Show version distribution if versions found
    $versionFiles = $mappings | Where-Object { $_.Version -ne "Unknown" }
    if ($versionFiles.Count -gt 0) {
        $versionCounts = $versionFiles | Group-Object -Property Version | Sort-Object Name
        Write-Host ""
        Write-Host "Version distribution:" -ForegroundColor Cyan
        foreach ($group in $versionCounts) {
            Write-Host "  v$($group.Name): $($group.Count) files" -ForegroundColor White
        }
    }
...
function Show-FilePatterns {
    ...
    foreach ($file in $files) {
        ...
        $extensions += $file.Extension.ToLower()
    ...
    Write-Host "Found file types: $($uniqueExtensions -join ', ')" -ForegroundColor Cyan

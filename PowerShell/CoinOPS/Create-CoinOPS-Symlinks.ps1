# CoinOPS Symlink Script - WITH CUSTOM PATH INPUT - ROMs and Videos Only
param(
...
# Function to get custom source path
function Get-CustomSourcePath {
    param(
        [string]$Type,
        [string]$PlatformName,
        [string]$ExpectedPath
    ...
    Write-Host ""
    Write-Host "CUSTOM $Type PATH NEEDED" -ForegroundColor Yellow
    Write-Host "Expected path not found: $ExpectedPath" -ForegroundColor Red
    Write-Host ""
    Write-Host "Please enter the correct $Type source path for $PlatformName"
    Write-Host "Examples:" -ForegroundColor Gray
    
    if ($Type -eq "ROM") {
        Write-Host "  D:\Arcade\System roms\Nintendo Pokimon Mini\roms" -ForegroundColor Gray
        Write-Host "  D:\Arcade\System roms\NES\roms" -ForegroundColor Gray
        Write-Host "  D:\Games\Pokemon Mini\roms" -ForegroundColor Gray
    } else {
        Write-Host "  C:\Arcade\LaunchBox\Videos\Nintendo Pokemon Mini" -ForegroundColor Gray
        Write-Host "  C:\Arcade\LaunchBox\Videos\Pokemon Mini" -ForegroundColor Gray
        Write-Host "  C:\LaunchBox\Videos\Nintendo Pokemon Mini" -ForegroundColor Gray
    }
    
    do {
        $customPath = Read-Host "`nEnter $Type source path (or 'skip' to skip this $Type)"
        
        if ($customPath.ToLower() -eq "skip") {
            Write-Host "Skipping $Type for $PlatformName" -ForegroundColor Yellow
            return $null
        }
        
        if ([string]::IsNullOrWhiteSpace($customPath)) {
            Write-Host "Please enter a path or type 'skip'" -ForegroundColor Red
            continue
        }
        ...
        if (Test-Path $customPath) {
            Write-Host "Path verified: $customPath" -ForegroundColor Green
            return $customPath
        } else {
            Write-Host "Path not found: $customPath" -ForegroundColor Red
            $retry = Get-UserConfirmation "Try again?"
            if (-not $retry) {
                return $null
            }
        }
...
# Enhanced verification function with custom path input
function Test-SymlinkSetup {
    ...
    $romSourceExists = Test-Path $systemRoms
    Write-Host "    ROM source: $(if ($romSourceExists) {'EXISTS'} else {'MISSING - CHECK PATH!'})" -ForegroundColor $(if ($romSourceExists) {'Green'} else {'Red'})
    
    # Ask for custom ROM path if needed
    $actualRomSource = $systemRoms
    if (-not $romSourceExists -and -not (Test-IsSymlink $coinopsRoms)) {
        $customRomPath = Get-CustomSourcePath -Type "ROM" -PlatformName $PlatformName -ExpectedPath $systemRoms
        if ($customRomPath) {
            $actualRomSource = $customRomPath
            $romSourceExists = $true
            Write-Host "    Using custom ROM source: $actualRomSource" -ForegroundColor Cyan
        }
    }
    ...
    Write-Host "    Video source: $(if ($videoSourceExists) {'EXISTS'} else {'MISSING - CHECK PATH!'})" -ForegroundColor $(if ($videoSourceExists) {'Green'} else {'Red'})
    
    # Ask for custom Video path if needed
    $actualVideoSource = $launchboxVideo
    if (-not $videoSourceExists -and -not (Test-IsSymlink $coinopsVideo)) {
        $customVideoPath = Get-CustomSourcePath -Type "VIDEO" -PlatformName $PlatformName -ExpectedPath $launchboxVideo
        if ($customVideoPath) {
            $actualVideoSource = $customVideoPath
            $videoSourceExists = $true
            Write-Host "    Using custom VIDEO source: $actualVideoSource" -ForegroundColor Cyan
        }
    }
    ...
    return @{
        ...
        SystemRoms = $actualRomSource
        LaunchBoxVideo = $actualVideoSource
        RomsIsSymlink = (Test-IsSymlink $coinopsRoms)
        VideoIsSymlink = (Test-IsSymlink $coinopsVideo)
        RomsSourceExists = $romSourceExists
        VideoSourceExists = $videoSourceExists
    }
}
...
# Main script
Clear-Host
Write-Host "=== CoinOPS Symlink Script - WITH CUSTOM PATH INPUT ===" -ForegroundColor Cyan
Write-Host "ROMs and Videos Only - Enhanced with Custom Source Paths" -ForegroundColor Gray

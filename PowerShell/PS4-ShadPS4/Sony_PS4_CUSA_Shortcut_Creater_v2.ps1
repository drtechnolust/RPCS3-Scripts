...
.DESCRIPTION
    ...
    games in fullscreen by default using the v0.15.0+ --fullscreen flag. Points to the
    fixed Pre-release folder maintained by the ShadPS4 Qt Launcher, which overwrites
    this folder in-place on every nightly update so shortcuts never need regenerating
    after an update. Includes a dry run mode to preview all changes before committing
    them. Unicode game titles (Japanese, etc.) are handled correctly via a temp-file
    save workaround that bypasses the WScript.Shell ANSI filename limitation.
#>
param(
    ...
    [string]$ShortcutPath = "D:\Arcade\System roms\Sony Playstation 4\PS4 Shortcuts",
...
Get-ChildItem -Path $RootPath -Directory | ForEach-Object {
    ...
    # Determine shortcut name with de-duplication
    $shortcutFull = $null
    $shortcutName = $null
    $existing     = $false
    ...
    # Build arguments - v0.15.0+ CLI syntax
    # Note: set fullscreen via shadPS4 Settings > Graphics > Display Mode = Fullscreen (Borderless)
    # rather than via CLI flag, as the --fullscreen flag can cause immediate exit on some nightly builds
    $arguments = "-g `"$($eboot.FullName)`""

    if ($WhatIf) {
        if ($existing) {
            Write-Host "[DRY RUN] Would UPDATE shortcut: $shortcutName" -ForegroundColor Cyan
        } else {
            Write-Host "[DRY RUN] Would CREATE shortcut: $shortcutName" -ForegroundColor Green
        }
    ...
    try {
        ...
        if (Test-Path $shortcutFull) {
            ...
            # Wait until file is confirmed gone before moving temp file
            $retries = 0
        ...
        if ($existing) {
            ...
            Write-Host "Updated shortcut: $shortcutName" -ForegroundColor Cyan
        } else {
